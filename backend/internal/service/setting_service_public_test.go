...
type settingPublicBrandingResolverStub struct {
	orgByHost map[string]*DistributionOrganization
}

func (s *settingPublicBrandingResolverStub) GetByBrandHost(ctx context.Context, host string) (*DistributionOrganization, error) {
	if s == nil {
		return nil, nil
	}
...
func TestSettingService_GetPublicSettings_AppliesDistributionBrandingOverride(t *testing.T) {
	repo := &settingPublicRepoStub{
		values: map[string]string{
	...
	svc := NewSettingService(repo, &config.Config{})
	svc.SetDistributionBrandingResolver(&settingPublicBrandingResolverStub{
		orgByHost: map[string]*DistributionOrganization{
			"brand.example.com": {
				ID:   88,
				Type: "oem",
	...
	ctx := WithPublicSettingsRequestMeta(context.Background(), "brand.example.com", "https")
