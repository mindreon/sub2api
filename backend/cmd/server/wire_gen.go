...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	settingService := service.ProvideSettingService(settingRepository, groupRepository, proxyRepository, distributionOrganizationRepository, configConfig)
	...
	distributionChannelBillingService := service.ProvideDistributionChannelBillingService(distributionAttributionRepository, distributionOrganizationRepository, distributionWalletRepository)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, distributionChannelBillingService, concurrencyService)
	...
	distributionPromotionService := service.ProvideDistributionPromotionService(distributionPromotionRepository, distributionMemberRepository, distributionAttributionRepository, distributionOrganizationRepository)
	...
	catalogModelHandler := admin.NewCatalogModelHandler(catalogModelService)
	voucherService := handler.ProvideVoucherService(client, settingRepository)
	voucherHandler := admin.NewVoucherHandler(voucherService)
	complianceHandler := admin.NewComplianceHandler(settingService)
	taskStore := handler.ProvideMediaTaskStore(client)
	configStore := handler.ProvideMediaConfigStore(settingRepository)
	mediaHandler := admin.NewMediaHandler(taskStore, configStore)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, adminDistributionHandler, catalogModelHandler, voucherHandler, complianceHandler, mediaHandler)
	...
	handlerVoucherHandler := handler.NewVoucherHandler(voucherService)
	...
	holdStore := handler.ProvideMediaHoldStore(client)
	balanceReader := handler.ProvideMediaBalanceReader(billingCacheService, holdStore)
	quoter := handler.ProvideMediaQuoter(balanceReader, configStore)
	charger := handler.ProvideMediaCharger(usageBillingRepository, usageLogRepository, billingCacheService, apiKeyRepository, accountRepository, groupRepository, userSubscriptionRepository, apiKeyAuthCacheInvalidator, deferredService)
	assetStore := handler.ProvideMediaAssetStore(settingRepository)
	reservation := handler.ProvideMediaReservation(client)
	ledger := handler.ProvideMediaLedger(quoter, charger, taskStore, holdStore, balanceReader, assetStore, reservation, billingCacheService)
	accountSelector := handler.ProvideMediaAccountSelector(gatewayService)
	accountCredentialsLoader := handler.ProvideMediaAccountLoader(accountRepository)
	providerFactory := handler.ProvideMediaProviderFactory()
	subscriptionResolver := handler.ProvideMediaSubscriptionResolver(groupRepository, userSubscriptionRepository)
	taskService := handler.ProvideMediaTaskService(ledger, accountSelector, accountCredentialsLoader, providerFactory, subscriptionResolver)
	handlerMediaHandler := handler.NewMediaHandler(taskService, taskStore, assetStore)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	poller := handler.ProvideMediaPoller(taskStore, ledger, accountCredentialsLoader, providerFactory, subscriptionResolver)
	handlers := handler.ProvideHandlers(authHandler, distributionHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, handlerVoucherHandler, availableChannelHandler, batchImageHandler, publicCatalogHandler, handlerMediaHandler, idempotencyCoordinator, idempotencyCleanupService, poller)
	...
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, schedulerSnapshotService, tokenRefreshService, accountExpiryService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, distributionAutoSettlementService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, userPlatformQuotaUsageFlusher)
