...
type UserQuery struct {
	...
	withVoucherOrders         *VoucherOrderQuery
...
// QueryVoucherOrders chains the current query on the "voucher_orders" edge.
func (_q *UserQuery) QueryVoucherOrders() *VoucherOrderQuery {
	query := (&VoucherOrderClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withVoucherOrders:         _q.withVoucherOrders.Clone(),
...
// WithVoucherOrders tells the query-builder to eager-load the nodes that are connected to
// the "voucher_orders" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithVoucherOrders(opts ...func(*VoucherOrderQuery)) *UserQuery {
	query := (&VoucherOrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withVoucherOrders = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withVoucherOrders; query != nil {
		if err := _q.loadVoucherOrders(ctx, query, nodes,
			func(n *User) { n.Edges.VoucherOrders = []*VoucherOrder{} },
			func(n *User, e *VoucherOrder) { n.Edges.VoucherOrders = append(n.Edges.VoucherOrders, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadVoucherOrders(ctx context.Context, query *VoucherOrderQuery, nodes []*User, init func(*User), assign func(*User, *VoucherOrder)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(voucherorder.FieldUserID)
	}
	query.Where(predicate.VoucherOrder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.VoucherOrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
