...
const typeOptions = computed(() => [
  ...
  { value: 'distribution_balance', label: t('admin.users.typeDistributionBalance') },
...
const isBalanceType = (type: string) => type === 'balance' || type === 'admin_balance' || type === 'affiliate_balance' || type === 'distribution_balance'
...
const getItemTitle = (item: BalanceHistoryItem) => {
  switch (item.type) {
    ...
    case 'distribution_balance':
      return item.value >= 0 ? t('redeem.balanceAddedDistribution') : t('redeem.balanceDeductedDistribution')
    case 'admin_balance':
