...
type MediaPricingFormState = {
  platform: GroupPlatform;
  allow_media_generation: boolean;
  media_rate_independent: boolean;
  media_rate_multiplier: number;
};
...
const resetDisabledMediaPricing = (
  form: Pick<MediaPricingFormState, "platform" | "allow_media_generation" | "media_rate_independent" | "media_rate_multiplier">,
) => {
  if (!supportsMediaPricingPlatform(form.platform)) {
    form.allow_media_generation = false;
    form.media_rate_independent = false;
    form.media_rate_multiplier = 1;
    return;
  }
  if (!form.allow_media_generation) {
    form.media_rate_independent = false;
    form.media_rate_multiplier = 1;
  }
};
...
const handleCreateGroup = async () => {
  ...
  try {
    ...
    resetDisabledMediaPricing(requestData);
    requestData.media_rate_multiplier = normalizeRateMultiplier(
...
const closeEditModal = () => {
  ...
  editForm.allow_media_generation = false;
  editForm.media_rate_independent = false;
  editForm.media_rate_multiplier = 1;
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    resetDisabledMediaPricing(payload);
    payload.media_rate_multiplier = normalizeRateMultiplier(
...
watch(
  ...
  (newVal) => {
    ...
    resetDisabledMediaPricing(createForm);
    resetModelsListState(createModelsListState);
...
watch(
  () => createForm.allow_media_generation,
  () => {
    resetDisabledMediaPricing(createForm);
  },
);
...
watch(
  ...
  (newVal) => {
    ...
    resetDisabledMediaPricing(editForm);
...
watch(
  () => editForm.allow_media_generation,
  () => {
    resetDisabledMediaPricing(editForm);
  },
);
